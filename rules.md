...
*   **[SACRIFICE] Create Whirlpool:** Sacrifices a point to create a vortex that pulls nearby points toward its center. If no points are nearby when created, the action instead creates a small, temporary fissure on the map. If no points are nearby when created, the action instead creates a small, temporary fissure on the map.
